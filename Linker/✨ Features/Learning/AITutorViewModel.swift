...
class AITutorViewModel: ObservableObject {
    ...
    func submitAnswer() async {
        ...
        do {
            let guestQuestion = GuestQuestion(
                id: question.id.uuidString,
                type: GuestQuestion.QuestionType(rawValue: question.type) ?? .translation,
            ...
            let submissionRequest = GuestAnswerSubmissionRequest(question: guestQuestion, userAnswer: userAnswer)
            let feedbackResponse = try await apiService.submitGuestAnswer(request: submissionRequest)
...
extension AITutorViewModel {
    ...
    private func syncKnowledgePointsToServer() async {
        ...
        for (errors, questionData, userAnswer) in knowledgePointsData {
            do {
                let questionDataStruct = QuestionData(from: questionData)
                let finalizationRequest = KnowledgePointFinalizationRequest(
                    errorAnalyses: errors,
                    questionData: questionDataStruct,
                    userAnswer: userAnswer
                )
                let savedCount = try await apiService.finalizeKnowledgePoints(request: finalizationRequest)
