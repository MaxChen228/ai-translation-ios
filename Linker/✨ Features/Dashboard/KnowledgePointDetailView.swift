...
// 位置：Linker/✨ Features/Dashboard/KnowledgePointDetailView.swift
...
struct KnowledgePointDetailView: View {
    ...
    private var relatedKnowledgePointsSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            // 相關知識點列表
            if let id = point.numericId, id < 0 || id > Int32.max {
                // 本地知識點提示
                HStack {
                ...
                VStack(spacing: 12) {
                    ForEach(relatedKnowledgePoints.prefix(5), id: \.effectiveId) { relatedPoint in
    ...
    private func performAIReview() {
        Task {
            ...
            do {
                guard let pointId = point.numericId else { return }
                let result = try await UnifiedAPIService.shared.aiReviewKnowledgePoint(id: pointId)
    ...
    private func toggleArchiveStatus() {
        Task {
            ...
            do {
                if localIsArchived {
                    try await UnifiedAPIService.shared.archiveKnowledgePoint(
                        compositeId: point.compositeId,
                        legacyId: point.numericId
                    )
                } else {
                    try await UnifiedAPIService.shared.unarchiveKnowledgePoint(
                        compositeId: point.compositeId,
                        legacyId: point.numericId
                    )
                }
    ...
    private func deleteKnowledgePoint() {
        Task {
            // 訪客功能已移除，只處理雲端知識點刪除
            do {
                try await UnifiedAPIService.shared.deleteKnowledgePoint(
                    compositeId: point.compositeId,
                    legacyId: point.numericId
                )
    ...
    private func loadRelatedKnowledgePoints() {
        ...
        if pointId < 0 || pointId > Int32.max {
            print("⚠️ 本地知識點無法載入相關連結，ID: \(pointId)")
        ...
        Task {
            ...
            do {
                let related = try await UnifiedAPIService.shared.getRelatedKnowledgePoints(id: pointId)
...
#Preview {
    ...
    let samplePoint = KnowledgePoint(
        compositeId: CompositeKnowledgePointID(userId: 1, sequenceId: 1),
        legacyId: 1,
        oldId: nil,
        category: "Grammar",
        subcategory: "Tense",
