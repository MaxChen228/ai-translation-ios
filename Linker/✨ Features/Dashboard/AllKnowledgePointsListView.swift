...
struct AllKnowledgePointsListView: View {
    ...
    // @State 用於儲存視圖的內部狀態
    @State private var selection = Set<String>() // 追蹤被選中的項目 ID (使用 effectiveId)
    ...
    private func batchArchiveAction() async {
        let selectedEffectiveIDs = Array(selection)
        
        do {
            // 找到對應的知識點，提取其複合ID或舊版ID
            for effectiveID in selectedEffectiveIDs {
                if let point = points.first(where: { $0.effectiveId == effectiveID }) {
