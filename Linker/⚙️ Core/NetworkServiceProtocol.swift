...
extension NetworkServiceProtocol {
    ...
    func logRequest(_ endpoint: APIEndpoint, duration: TimeInterval? = nil) {
        ...
        if let duration = duration {
            Logger.info("\(logMessage) - Duration: \(String(format: "%.2f", duration))s", category: .network)
        } else {
    ...
    func logError(_ endpoint: APIEndpoint, error: Error) {
        Logger.error("API Error: \(endpoint.method.rawValue) \(endpoint.urlPath) - \(error.localizedDescription)", category: .network)
    }
