...
protocol UnifiedAPIServiceProtocol {
    ...
    func updateKnowledgePoint(id: Int, updates: [String: Any]) async throws
    ...
    func finalizeKnowledgePoints(errors: [ErrorAnalysis], questionData: [String: Any?], userAnswer: String) async throws -> Int
    ...
    func submitGuestAnswer(question: [String: Any], answer: String) async throws -> FeedbackResponse
...
extension UnifiedAPIService {
    ...
    func fetchKnowledgePoint(id: Int) async throws -> KnowledgePoint {
        return try await request(.fetchKnowledgePoint(id: id), responseType: KnowledgePoint.self)
    }
    
    func updateKnowledgePoint(id: Int, updates: [String: Any]) async throws {
        try await requestWithoutResponse(.updateKnowledgePoint(id: id, updates: updates))
    }
    
    func deleteKnowledgePoint(id: Int) async throws {
        try await requestWithoutResponse(.deleteKnowledgePoint(id: id))
    }
    
    func archiveKnowledgePoint(id: Int) async throws {
        try await requestWithoutResponse(.archiveKnowledgePoint(id: id))
    }
    
    func unarchiveKnowledgePoint(id: Int) async throws {
        try await requestWithoutResponse(.unarchiveKnowledgePoint(id: id))
    }
    ...
    func batchArchiveKnowledgePoints(ids: [Int]) async throws {
        try await requestWithoutResponse(.batchOperationKnowledgePoints(action: "archive", ids: ids))
    }
    ...
    func aiReviewKnowledgePoint(id: Int, modelName: String? = nil) async throws -> AIReviewResult {
        ...
        let response = try await request(.aiReviewKnowledgePoint(id: id, modelName: modelName), 
    ...
    func finalizeKnowledgePoints(errors: [ErrorAnalysis], 
        ...
        do {
            let response = try await request(.finalizeKnowledgePoints(errors, questionData, userAnswer), 
                                           responseType: FinalizeResponse.self)
            return response.saved_count
        } catch {
            // API不可用時的降級策略
            print("⚠️ Finalize API不可用，使用本地備用策略")
            return try await handleFinalizeOffline(errors: errors, questionData: questionData, userAnswer: userAnswer)
        }
    }
    ...
    func submitGuestAnswer(question: [String: Any], answer: String) async throws -> FeedbackResponse {
        return try await request(.submitGuestAnswer(question, answer), responseType: FeedbackResponse.self)
    }
    ...
    func getRelatedKnowledgePoints(id: Int, includeDetails: Bool = true) async throws -> [KnowledgePoint] {
        let response = try await request(.getRelatedKnowledgePoints(id: id, includeDetails: includeDetails), 
            ...
            if let details = connection.connectedPointDetails,
               !seenIds.contains(connection.connectedPointId) {
                ...
                let knowledgePoint = KnowledgePoint(
                    id: connection.connectedPointId,
...
private extension UnifiedAPIService {
    ...
    func handleFinalizeOffline(errors: [ErrorAnalysis], 
                              questionData: [String: Any?], 
                              userAnswer: String) async throws -> Int {
        // 使用GuestDataManager作為備用
        let guestDataManager = GuestDataManager.shared
        
        for error in errors {
            let knowledgePointData: [String: Any] = [
                "id": UUID().uuidString,
                "category": error.categoryName,
                "subcategory": error.errorTypeCode,
                "correct_phrase": error.correction,
                "explanation": error.explanation,
                "user_context_sentence": userAnswer,
                "incorrect_phrase_in_context": error.originalPhrase,
                "key_point_summary": error.keyPointSummary,
                "mastery_level": 0.0,
                "mistake_count": 1,
                "correct_count": 0,
                "created_at": ISO8601DateFormatter().string(from: Date()),
                "is_archived": false,
                "storage_type": "api_fallback"
            ]
            
            guestDataManager.saveGuestKnowledgePoint(knowledgePointData)
        }
